        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            ...
            <li style="text-align: right" class="pr">
              ...
              <a href="javascript:;" class="register" @click="toRegister">注册 TBMall 账号</a>
              <a
                style="padding: 1px 0 0 10px"
                @click="open('找回密码','请联系作者邮箱找回密码或使用测试账号登录：test | test')"
              >忘记密码 ?</a>
            </li>
          </ul>
          <!--登陆-->
...
export default {
  ...
  methods: {
    ...
    login () {
      ...
      // }
      var params = {
        typeInName: this.ruleForm.userName,
        typeInPassword: this.ruleForm.userPwd,
      ...
      // setStore('userId', id)
      // store.commit('RECORD_USERINFO', { info: { username: params.typeInName } })
      // this.$router.push({
      //   path: '/'
      // })
      ...
      userLogin(params).then(res => {
        console.log(res)
        if (res.success) {
          setStore('userId', res.id)
          setStore('login', true)
          store.commit('RECORD_USERINFO', { info: { username: params.typeInName, file: 'https://avatars1.githubusercontent.com/u/32808034?s=460&v=4' } })
