        ...
        <div slot="content">
          <ul class="address-item-list clearfix">
            <li
              ...
              :key="i"
              class="address pr"
              :class="{checked:streetName === item.address && userName === item.name && tel === item.phonenum}"
              @click="chooseAddress(item.addressId, item.userName, item.tel, item.streetName)"
            >
              <span
                v-if="streetName === item.address && userName === item.name && tel === item.phonenum"
                class="pa"
              >
...
<script>
import { cartDel,getCartList, addressList, addressUpdate, addressAdd, addressDel, productDet, submitOrder } from '/api/goods'
...
export default {
  ...
  methods: {
    ...
    _getCartList () {
      ...
      //   result.forEach(e => {
      //     e.productNum = Number(e.productNum)
      //     e.checked = '1'
      //   })
      ...
      addressList({ userId: this.userId }).then(res => {
        ...
        if (data.length) {
          data.forEach(element => {
            ...
            element.streetName = element.address,
              element.tel = element.phonenum
          });
    ...
    _addressUpdate (item) {
      var params = {
        userId: item.userId,
        oldName: this.oldItem.userName,
        oldAddress: this.oldItem.streetName,
        oldPhoneNum: this.oldItem.tel,
        newName: item.userName,
        newAddress: item.streetName,
        newPhoneNum: item.tel
      }
    ...
    _submitOrder () {
      ...
      for (var i = 0; i < this.cartList.length; i++) {
        if (this.cartList[i].checked === '1') {
          // this.cartList[i].productNum = Number(this.cartList[i].productNum)
          // array.push(this.cartList[i])
          array.push({
            goodId: this.cartList[i].productId,
            goodNum: Number(this.cartList[i].productNum)
          })
        }
      }
      var params = {
        ...
        goodsList: array,
        orderTotal: String(this.orderTotal)
      }
    ...
    _productDet (productId) {
      productDet({ productId }).then(res => {
        ...
        item.productName = item.goodName
