          ...
          <div class="nav-sub-wrapper" :class="{fixed:st}">
            ...
            >
              ...
              <el-submenu
                ...
                :index="item.index"
              >
                <template slot="title">{{item.kindName}}</template>
                <el-menu-item
                  :index="childItem.index"
                  v-for="(childItem,ci) in item.child"
                  v-if="!childItem.child.length"
                >{{childItem.kindName}}</el-menu-item>
                ...
                <el-submenu
                  ...
                  v-if="childItem.child.length"
                >
                  <template slot="title">{{childItem.kindName}}</template>
                  <el-menu-item
                    :index="ccItem.index"
                    v-for="(ccItem,cci) in childItem.child"
                  >{{ccItem.kindName}}</el-menu-item>
                </el-submenu>
              </el-submenu>
...
const ws = 'ws://118.25.149.183:8080/websocket/chat'
...
export default {
  data () {
    return {
      ...
      socket: {
        // WebSocket 组件对象
        io: null, // WebSocket 连接对象
        running: false // WebSocket 连接状态标识
      }
    }
  },
  computed: {
    ...mapState([
      'cartList', 'receiveInCart', 'showCart'
    ]),
    // 计算价格
    totalPrice () {
      var totalPrice = 0
  ...
  methods: {
    ...
    _getNavList () {
      navList().then(res => {
        ...
        result.forEach(element => {
          element.indexArr = element.index.split('-')
          if (element.indexArr.length === 1) {
            element.child = []
            navList.push(element)
          }
        })
        ...
        result.forEach(element => {
          element.indexArr = element.index.split('-')
          if (element.indexArr.length === 2) {
            navList.forEach(nav => {
              if (element.indexArr[0] === nav.index) {
                element.child = []
                nav.child.push(element)
              }
            })
          }
        })
            ...
            navList.forEach(nav => {
              ...
              if (nav.child.length) {
                ...
                nav.child.forEach(child => {
                  ...
                  // console.log(element.indexArr[0] + '-' + element.indexArr[1])
                  if (child.index === element.indexArr[0] + '-' + element.indexArr[1]) {
                    child.child.push(element)
                    // console.log(element)
                  }
                })
              }
    ...
    //WebSocket 
     // WebSocket 组件 - 启动器
    initSocket() {
      // 如果 WebSocket 已在运行中，终止方法进行
      if (this.socket.running) return;

      // 创建 WebSocket 连接
      this.socket.io = new WebSocket(ws);
      // 创建一个对象引用
      ...
      // 处理器 - 收到消息
      io.onmessage = event => {
        this.dealMessage(event);

      };

      // 处理器 - 发生错误 ( 发生错误后会自动关闭连接，触发 onclose 处理器 )
      io.onerror = () => {
        log("error");
      };
